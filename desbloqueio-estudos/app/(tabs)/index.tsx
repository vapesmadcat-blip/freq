    ...
    return (
      <ScreenContainer className="px-5 pt-5" edges={["top", "left", "right", "bottom"]}>
        <View style={styles.quizHeader}>
          ...
          <Text style={[styles.quizMeta, { color: colors.muted }]}>{question.subject} · {question.source === "api" ? "API ENEM" : "Banco local"}</Text>
        </View>
        ...
        <View style={styles.questionArea}>
          ...
          {answered !== undefined && <View style={[styles.feedback, { backgroundColor: answered ? colors.success + "18" : colors.error + "15" }]}><Text style={[styles.feedbackTitle, { color: answered ? colors.success : colors.error }]}>{answered ? "Mandou bem!" : "Quase lá — siga estudando"}</Text><Text style={[styles.feedbackText, { color: colors.foreground }]}>{question.explanation}</Text></View>}
        </View>
